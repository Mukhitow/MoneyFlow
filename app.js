...
const sample = {
  incomes: [{id:'i1',name:'Зарплата',amount:420000,day:15},{id:'i2',name:'Аванс',amount:200000,day:1}],
  bills: [{id:'b1',name:'Аренда',amount:180000,day:25,priority:10},{id:'b2',name:'Коммуналка',amount:25000,day:20,priority:9},{id:'b3',name:'Интернет',amount:6000,day:10,priority:8},{id:'b4',name:'Подписки',amount:3000,day:12,priority:5}],
  loans: [{id:'l1',name:'Кредит карта',balance:350000,apr:34.9,minPayment:20000,day:27},{id:'l2',name:'Потреб кредит',balance:900000,apr:21.0,minPayment:35000,day:5}],
  goals: [{id:'g1',name:'Подушка',target:1000000,monthly:50000},{id:'g2',name:'Отпуск',target:800000,monthly:70000}],
  startBalance: 50000
};
...
function plan({ startBalance, incomes, bills, loans, goals, strategy='avalanche' }){
  ...
  let balance = startBalance; let loanBalances = Object.fromEntries(loans.map(l=>[l.id,l.balance])); const tl=[]; let extra=0;
  for (const d of days){
    for (const infl of d.inflow){ balance+=infl.amount; tl.push({day:d.day, type:'+', name: infl.name, amount: infl.amount, balance}); }
    for (const o of d.outflow){
      if (o.type==='loanMin'){ const can=Math.min(balance,o.amount); balance-=can; loanBalances[o.id]-=can; tl.push({day:d.day,type:'-',name:`${o.name} (мин.)`,amount:can,balance}); if (can<o.amount) tl.push({day:d.day,type:'!',name:`${o.name}: не хватило на мин. платёж`,amount:o.amount-can,balance}); }
      else { const can=Math.min(balance,o.amount); balance-=can; tl.push({day:d.day,type:'-',name:o.name,amount:can,balance}); if (can<o.amount) tl.push({day:d.day,type:'!',name:`${o.name}: частично оплачено`,amount:o.amount-can,balance}); }
    }
    const got = d.inflow.reduce((s,i)=>s+i.amount,0);
    if (got>0) extra += Math.floor(got * 0.25);
    if (extra>0 && d.day%2===0){
      const order = Object.entries(loanBalances).map(([id,bal])=>({id,bal,apr:loans.find(l=>l.id===id).apr})).filter(x=>x.bal>0).sort((a,b)=> strategy==='avalanche' ? (b.apr-a.apr || a.bal-b.bal) : (a.bal-b.bal || b.apr-a.apr));
      let toUse = Math.min(balance, extra);
      for (const t of order){ if (toUse<=0) break; const pay=Math.min(toUse,t.bal); if (pay>0){ balance-=pay; toUse-=pay; loanBalances[t.id]-=pay; const ln=loans.find(l=>l.id===t.id); tl.push({day:d.day,type:'-',name:`${ln.name} (ускор.)`,amount:pay,balance}); } }
...
function Card({children, title, right}){
  return <div className="bg-white/80 backdrop-blur border shadow-soft rounded-2xl p-4">
...
function Incomes({items, setItems}){
  ...
  return <Card title="Доходы" right={<button className="px-3 py-2 border rounded-xl">+ добавить</button>}>
    <div className="space-y-2">
      {items.map(i => <div key={i.id} className="grid grid-cols-12 gap-2 items-center">
        ...
        <div className="col-span-3 flex items-center gap-2"><span className="text-sm text-slate-500">День</span><Day value={i.day} onChange={v=>up(i.id,{day:v})}/></div>
        <div className="col-span-1 text-right"><button className="text-red-600" onClick={()=>rm(i.id)}>x</button></div>
      </div>)}
    </div>
  </Card>;
}
...
function Bills({items, setItems}){
  ...
  const rm  = id => setItems(items.filter(i=>i.id!==id));
  const up  = (id, patch) => setItems(items.map(i=>i.id===id?{...i,...patch}:i));
  return <Card title="Обязательные платежи" right={<button className="px-3 py-2 border rounded-xl">+ добавить</button>}>
    <div className="space-y-2">
      {items.map(b => <div key={b.id} className="grid grid-cols-12 gap-2 items-center">
        ...
        <div className="col-span-3 flex items-center gap-2"><span className="text-sm text-slate-500">День</span><Day value={b.day} onChange={v=>up(b.id,{day:v})}/></div>
        <div className="col-span-1"><input type="number" className="w-full px-3 py-2 border rounded-xl" value={b.priority} min="1" max="10" onChange={e=>up(b.id,{priority:Number(e.target.value)})}/></div>
        <div className="col-span-1 text-right"><button className="text-red-600" onClick={()=>rm(b.id)}>x</button></div>
      </div>)}
    </div>
  </Card>;
}
...
function Loans({items, setItems}){
  ...
  const rm  = id => setItems(items.filter(i=>i.id!==id));
  const up  = (id, patch) => setItems(items.map(i=>i.id===id?{...i,...patch}:i));
  return <Card title="Кредиты" right={<button className="px-3 py-2 border rounded-xl">+ добавить</button>}>
    <div className="space-y-2">
      {items.map(l => <div key={l.id} className="grid grid-cols-12 gap-2 items-center">
        ...
        <div className="col-span-2 flex items-center gap-2"><span className="text-sm text-slate-500">День</span><Day value={l.day} onChange={v=>up(l.id,{day:v})}/></div>
        <div className="col-span-1 text-right"><button className="text-red-600" onClick={()=>rm(l.id)}>x</button></div>
      </div>)}
    </div>
...
function Timeline({tl}){
  ...
  return <div className="space-y-2">
    {tl.map((t, idx) => {
      ...
      return <div key={idx} className={`flex items-center justify-between border rounded-xl px-3 py-2 ${cls}`}>
...
function App(){
  ...
  const { tl, loanBalances, totals } = useMemo(()=>plan({ startBalance, incomes, bills, loans, goals, strategy }), [startBalance,incomes,bills,loans,goals,strategy]);

  const exportJSON = () => { const blob=new Blob([JSON.stringify({incomes,bills,loans,goals,startBalance},null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='moneyflow_data.json'; a.click(); URL.revokeObjectURL(a.href); };
  ...
  return <div className="max-w-6xl mx-auto p-4 md:p-8">
      ...
      <div className="flex items-center gap-2">
        ...
        <label className="px-3 py-2 rounded-xl border bg-white hover:bg-slate-50 cursor-pointer">Импорт
          <input type="file" accept="application/json" className="hidden" onChange={e=>{
            ...
            const r=new FileReader(); r.onload=()=>{ try{ const d=JSON.parse(r.result); if(d.incomes) setIncomes(d.incomes); if(d.bills) setBills(d.bills); if(d.loans) setLoans(d.loans); if(d.goals) setGoals(d.goals); if(typeof d.startBalance==='number') setStartBalance(d.startBalance);}catch(_){ alert('Не удалось импортировать файл'); } }; r.readAsText(file);
          }}/>
        </label>
      </div>
    </header>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      <div className="lg:col-span-3 space-y-4">
        ...
        <Card title="Цели накоплений" right={<button className="px-3 py-2 border rounded-xl" onClick={()=>setGoals([...goals,{id:uid(),name:'Цель',target:0,monthly:0}])}>+ добавить</button>}>
          <div className="space-y-2">
            {goals.map(g => <div key={g.id} className="grid grid-cols-12 gap-2 items-center">
              <input className="col-span-5 px-3 py-2 border rounded-xl" value={g.name} onChange={e=>setGoals(goals.map(x=>x.id===g.id?{...x,name:e.target.value}:x))}/>
              <div className="col-span-3"><Num value={g.target} onChange={v=>setGoals(goals.map(x=>x.id===g.id?{...x,target:v}:x))}/></div>
              <div className="col-span-3"><Num value={g.monthly} onChange={v=>setGoals(goals.map(x=>x.id===g.id?{...x,monthly:v}:x))}/></div>
              <div className="col-span-1 text-right"><button className="text-red-600" onClick={()=>setGoals(goals.filter(x=>x.id!==g.id))}>x</button></div>
            </div>)}
          </div>
        </Card>
      </div>
        ...
        <Card title="Итоги месяца">
          ...
          <div className="mt-3 text-sm">
            ...
            {Object.entries(loanBalances).map(([id, bal]) => {
              ...
              return <div key={id} className="flex items-center justify-between border rounded-xl px-3 py-2 mb-2">
