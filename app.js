...
function planMonth({ startBalance, incomes, bills, loans, goals, strategy='avalanche' }){
  ...
  for (const d of days){
    ...
    if (extraForLoans>0 && d.day % 2 === 0){
      const order = Object.entries(loanBalances).map(([id,bal])=>{
        const loan = loans.find(l=>l.id===id);
        return { id, bal, apr: loan.apr };
      }).filter(x=>x.bal>0).sort((a,b)=> strategy==='avalanche' ? (b.apr-a.apr || a.bal-b.bal) : (a.bal-b.bal || b.apr-a.apr));
...
function NumberInput({value, onChange}){
  return <input type="number" value={value} onChange={e=>onChange(Number(e.target.value))} step="100" />;
}
function DayInput({value, onChange}){
  return <input type="number" value={value} min={1} max={31} onChange={e=>onChange(clampDay(e.target.value))} />;
}
  ...
  return <Section title="Обязательные платежи" right={<button className="btn" onClick={add}>+ добавить</button>}>
    <div className="grid">
      {items.map(b => <div key={b.id} className="row">
        ...
        <div style={{textAlign:'right'}}><button className="btn" onClick={()=>rm(b.id)}>x</button></div>
      </div>)}
    </div>
  </Section>;
}
  ...
  return <Section title="Кредиты" right={<button className="btn" onClick={add}>+ добавить</button>}>
    <div className="grid">
      {items.map(l => <div key={l.id} className="row">
        ...
        <div><span className="muted">День</span> <DayInput value={l.day} onChange={v=>up(l.id,{day:v})} /></div>
      </div>)}
    </div>
...
function GoalsEditor({items, setItems}){
  ...
  const rm  = id => setItems(items.filter(i=>i.id!==id));
  const up  = (id,patch) => setItems(items.map(i=>i.id===id?{...i,...patch}:i));
  return <Section title="Цели накоплений" right={<button className="btn" onClick={add}>+ добавить</button>}>
    <div className="grid">
      {items.map(g => <div key={g.id} className="row">
        <input value={g.name} onChange={e=>up(g.id,{name:e.target.value})} />
        <NumberInput value={g.target} onChange={v=>up(g.id,{target:v})} />
        <NumberInput value={g.monthly} onChange={v=>up(g.id,{monthly:v})} />
        <div></div><div style={{textAlign:'right'}}><button className="btn" onClick={()=>rm(g.id)}>x</button></div>
      </div>)}
    </div>
  </Section>;
}
    ...
    {timeline.map((t,idx)=>{
      ...
      return <div key={idx} className={cls}>
        <div style={{display:'flex', gap:8, alignItems:'center'}}>
          <span className="pill">день {t.day}</span>
          <b>{t.name}</b>
        </div>
        <div style={{textAlign:'right'}}>
          <div><b>{t.type}{formatMoney(t.amount)}</b></div>
          <div className="muted">баланс: {formatMoney(t.balance)}</div>
        </div>
      </div>
    })}
  </div>;
...
function App(){
  ...
  const exportJSON = () => {
    const data = { incomes, bills, loans, goals, startBalance };
    const blob = new Blob([JSON.stringify(data,null,2)], {type:'application/json'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'moneyflow_data.json'; a.click();
  ...
  return (<div className="container">
    <header style={{display:'flex', alignItems:'center', justifyContent:'space-between', margin:'16px 0'}}>
      <div>
        <div className="title">MoneyFlow — PWA</div>
        <div className="muted">Планируй доходы, платежи, кредиты и цели. Симуляция на 1 месяц.</div>
      </div>
    ...
    <div className="grid grid-5">
      <div style={{gridColumn:'span 3'}} className="grid">
        <Section title="Начальный баланс на 1 число">
          <div style={{display:'flex', gap:12, alignItems:'center'}}>
            ...
            <div style={{display:'flex', gap:8, alignItems:'center'}} className="muted">
      ...
      <div style={{gridColumn:'span 2'}} className="grid">
        ...
        <Section title="Таймлайн операций"><Timeline timeline={timeline} /></Section>
      </div>
    </div>
